...
## [codeblock]
## func _ready() -> void:
##     var neural_network = NNET.new([1,3,2,1], 0.5, true)
...
var tfd: bool
## TR stands for train run, if variable is true neural network will call function run ( see [method NNET.run] ) before training starts, otherwise it will not. [br]
## see [method NNET.disable_train_run] and [method NNET.enable_train_run]
var tr_bool: bool = true

func _init(layers_construction: Array, learning_rate_a: float, use_bias: bool, range_a: RangeN = RangeN.R_0_1, afd_a : bool = false) -> void:
...
## enables train run
func enable_train_run() -> void:
	tr_bool = true

## disables train run
func disable_train_run() -> void:
	tr_bool = false
...
func set_desired_output(desired_output: Array[float]) -> void:
	...
	output = desired_output.duplicate()
	...
	for layer in range(last_layer - 1, 0, -1):
		for neuron in range(layers[layer]):
			...
			for after_neuron in range(layers[layer + 1]):
				deltas[layer][neuron] += deltas[layer + 1][after_neuron] * weights[weights[get_weight(layer, neuron, layer + 1, after_neuron)]]
...
##
## [code]neural_network.train(2)[/code]
## equals to
## [codeblock]
## var i : int = 0
## while i < someValue:
