...
func duplicate():
	...
	buffer.output.assign(output.duplicate(true))
	buffer.neurons_out[last_layer].assign(neurons_out[last_layer].duplicate(true))
	buffer.neurons_out[0].assign(neurons_out[0].duplicate(true))
	buffer.neurons_in[0].assign(neurons_in[0].duplicate(true))
	buffer.weights.assign(weights.duplicate(true))
...
func assign(buffer : NNET) -> void:
	...
	layers.assign(buffer.layers.duplicate(true))
	...
	weights.assign(buffer.weights.duplicate(true))
