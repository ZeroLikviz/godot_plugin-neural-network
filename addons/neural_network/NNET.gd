...
func assign(buffer : NNET) -> void:
	...
	neurons_in.assign(buffer.neurons_in)
	neurons_out.assign(buffer.neurons_out)
	biases.assign(buffer.biases)
	weights.assign(buffer.weights)
	deltas.assign(buffer.deltas)
	tfd = buffer.tfd
