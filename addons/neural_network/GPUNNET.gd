...
enum {run = 0, find_deltas = 1, train = 2, fill_table = 3}
...
func _init(layers_construction: Array[int] = [1,1], learning_rate_value: float = 1.0, use_bias: bool = true) -> void:
	...
	var bias_size : int = 0;    i = 1
	while i < layers_size:
		bias_size += layers[i]
		i += 1
	var weights_size : int = 0; i = 0
	while i < layers_size - 1:
		weights_size += layers[i] * layers[i + 1]; i += 1
	var neurons_size : int = bias_size + layers[0]
	...
	common_data            = device.storage_buffer_create(4                                * 4)
	bias_storage           = device.storage_buffer_create(bias_size * int(use_bias)        * 4)
	deltas_storage         = device.storage_buffer_create(x_component * y_component        * 4)
	...
	device.compute_list_dispatch(              compute_list, x_component, y_component, 1)
...
func fill_data() -> void:
	...
	common_data_pba.encode_s32(   4, last_layer    )
	common_data_pba.encode_s32(   8, layers_size   )
	common_data_pba.encode_s32(  12, run           )
