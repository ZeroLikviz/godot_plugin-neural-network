...
func set_input(input : Array[float]) -> void:
	assert(input.size() == layers[last_layer], "GPUNNET set_input LINE 149: Size of provided \"input\" doesn't match with the size of neural network's input. Provided input size: " + str(input.size()) + ". Neural network's input size: " + str(layers[0]) + ".")
...
func set_desired_output(output : Array[float]) -> void:
	assert(output.size() == layers[last_layer], "GPUNNET set_desired_output LINE 153: Size of provided \"output\" doesn't match with the size of neural network's output. Provided output size: " + str(output.size()) + ". Neural network's output size: " + str(layers[last_layer]) + ".")
