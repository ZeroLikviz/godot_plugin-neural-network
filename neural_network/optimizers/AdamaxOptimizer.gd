...
## Infinity norm for weights.
var u: Array[PackedFloat32Array] = []
## First moment for biases.
var m_bias: PackedFloat32Array = []
## Infinity norm for biases.
var u_bias: PackedFloat32Array = []
## Timestep counter.
var t: int = 0
...
func _init(lr: float = 0.002, b1: float = 0.9, b2: float = 0.999, eps: float = NetworkConstants.EPS) -> void:
	learning_rate = lr
	beta1 = b1
	beta2 = b2
	epsilon = eps
...
func apply_gradients(layer: LayerCPU) -> void:
	...
	for i in range(layer.neuron_activations.size()):
		for j in range(layer.next_layer_neuron_count):
			...
			u[i][j] = max(beta2 * u[i][j], abs(grad))
			var m_hat: float = m[i][j] / (1.0 - pow(beta1, t))
			layer.weights[i][j] -= learning_rate * m_hat / (u[i][j] + epsilon)
	if layer.biases.size() == 0:
		return
	for j in range(layer.next_layer_neuron_count):
		var grad: float = layer.bias_data[bias_grad_index][j]
		m_bias[j] = beta1 * m_bias[j] + (1.0 - beta1) * grad
		u_bias[j] = max(beta2 * u_bias[j], abs(grad))
		var m_hat: float = m_bias[j] / (1.0 - pow(beta1, t))
		layer.biases[j] -= learning_rate * m_hat / (u_bias[j] + epsilon)

func duplicate() -> Optimizer:
	return AdamaxOptimizer.new(learning_rate, beta1, beta2, epsilon)
