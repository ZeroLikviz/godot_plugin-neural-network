...
func apply_gradients(layer: LayerCPU) -> void:
	...
	t += 1
	var mu_t: float = beta1 * (1.0 - 0.5 * pow(0.96, t / 250.0))
	var mu_t1: float = beta1 * (1.0 - 0.5 * pow(0.96, (t + 1) / 250.0))
	for i in range(layer.neuron_activations.size()):
		for j in range(layer.next_layer_neuron_count):
			...
			v[i][j] = beta2 * v[i][j] + (1.0 - beta2) * grad * grad
			var m_hat: float = m[i][j] / (1.0 - pow(beta1, t))
			var v_hat: float = v[i][j] / (1.0 - pow(beta2, t))
			var m_bar: float = mu_t1 * m_hat + (1.0 - mu_t) * grad
			layer.weights[i][j] -= learning_rate * m_bar / (sqrt(v_hat) + epsilon)
	if layer.biases.size() == 0:
		return
	for j in range(layer.next_layer_neuron_count):
		var grad: float = layer.bias_data[bias_grad_index][j]
		m_bias[j] = beta1 * m_bias[j] + (1.0 - beta1) * grad
		v_bias[j] = beta2 * v_bias[j] + (1.0 - beta2) * grad * grad
		var m_hat: float = m_bias[j] / (1.0 - pow(beta1, t))
		var v_hat: float = v_bias[j] / (1.0 - pow(beta2, t))
		var m_bar: float = mu_t1 * m_hat + (1.0 - mu_t) * grad
		layer.biases[j] -= learning_rate * m_bar / (sqrt(v_hat) + epsilon)

func duplicate() -> Optimizer:
	return NadamOptimizer.new(learning_rate, beta1, beta1, epsilon)
